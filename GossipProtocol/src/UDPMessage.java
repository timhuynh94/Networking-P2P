...
public class UDPMessage 
{
	...
	public UDPMessage(ID id1, ID id2, TimeToLive ttl, String message)
	{
		this(id1, id2, ttl, message.getBytes());
	}
	public UDPMessage(ID id1, ID id2, TimeToLive ttl, byte[] message)
	{
		this.id1 = id1;
		this.id2 = id2;
		this.timeToLive = ttl;
	...
	public DatagramPacket getPacket()
	{
		...
		byte[] id2Bytes = new byte[ID.getIDLength()];
		byte[] ttlBytes = new byte[TimeToLive.getLengthInBytes()];
		byte[] full = new byte[(ID.getIDLength() * 2) + TimeToLive.getLengthInBytes() + /*Message length*/];
		int j;
